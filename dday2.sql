...
ALTER TABLE animals ALTER COLUMN species TYPE  VARCHAR (40); 
...
CREATE TABLE cart(
    ...
    user_id INTEGER NOT NULL REFERENCES cart_users(user_id),
    product_id INTEGER NOT NULL REFERENCES products(product_id),
    quantity INTEGER
);

INSERT INTO cart_users (user_name, user_email) 
VALUES('Blake', 'blakedyblake@gmail.com'), ('Adam', 'batman99@yahoo.com'), 
('Sam', 'SamIAm@gmail.com');

INSERT INTO products(product_name, product_price)
VALUES ('Carrots', 1.99),('Beans', 2.99),
('Popcorn', 3.99),('Root beer', 0.99),
('Sushi', 11.99);

INSERT INTO cart (user_id,product_id,quantity)
VALUES (1,5,7), (1,3,1), (2,2,2), (2,3,4), (3,1,1), (3,5,1);

SELECT * FROM products WHERE product_id IN(
  	SELECT product_id FROM cart WHERE user_id =1
);

SELECT product_name "Product", product_price "Price", quantity, user_name "Customer", user_email "Email" FROM cart_users


SELECT cart_id, user_name, product_name,product_price, quantity ,product_price * quantity FROM cart_users
JOIN cart ON cart_users.user_id = cart.user_id
JOIN products ON cart.product_id = products.product_id;

UPDATE cart SET quantity = 100 WHERE cart_id = 1;
